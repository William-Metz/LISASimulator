	...
	#tag Method, Flags = &h0
		Sub CalculateAmplitudes()
		  ...
		  If Parameters.PNOrder > 0 Then
		    ...
		    A(133) = -δ*45/8*c1p2*s2β*s1p4
		    A(134) = δ*4.5*c2β*c1*s1p5
		    A(135) = δ*1.125*s2β*s1p6
		    A(136) = δ*(-1/64 *cβ*sβ + 43/128 *cβ*c2*sβ - 23/128 *c4*s2β + 5/256 *c6*s2β)
		    A(137) = δ*((-1 -1/4 *c2β)*c1 + 1/4 *c2β*c1*c2)*s1p3
		    A(138) = δ*(0.125 *c1p2*s2β*s1p4)
		    A(139) = δ*(0.5 *sβ2*s4)
		    A(140) = δ*(1/64 *cβ*sβ + 43/128 *cβ*c2*sβ + 23/128 *c4*s2β + 5/256 *c6*s2β)
		    A(141) = δ*((-1 - 0.25 *c2β)*c1p3 - 0.25*c2β*c1p3*c2)*s1
		    A(142) = -δ*(0.125 *c1p4*s2β*s1p2)
		    A(143) = δ*(45/8 *c1p4*s2β*s1p2)
		    A(144) = δ*(4.5*c2β*c1p5*s1)
		    A(145) = -δ*(1.125 *c1p6*s2β)
		  ...
		  If Parameters.PNOrder > 1 Then
		    ...
		    A(24) = 0.5*(-(1 + 1/3 *c2β) + η*(3 + c2β))*c1p6*sβ2*s1p2
		    ...
		    +(c1*(-6*s2β - 31/12*c2*s2β - 1/12*c4*s2β + 19/48*s4β) + 7/24*c3*s4β + 7/48*c5*s4β)*s1p3            
		    ...
		    A(151) = (47/8 *cβ + c3β/8 + (7/6 *cβ + c3β/6)*c2 - (1/24 *cβ + 7/24 *c3β)*c4 _
		    ...
		    A(162) = 8*(η - 1/3)*cβ*c1p8*sβ2
		    
		    A(163) = 0.5*χayDN*(1 + c2)
		    A(164) = χayDN*s1p2
		    A(165) = 0.5*(χaxDN*(cβ2 - cβ2*c2) + χazDN*(-cβ*sβ + cβ*c2*sβ))
		    A(166) = χaxDN*cβ*sβ*s2 - χazDN*sβ2*s2
		    A(167) = 0.5*(χaxDN*(cβ2 + cβ2*c2) + χazDN*(-cβ*sβ - cβ*c2*sβ))
		    A(168) = 0.5*δ*(χsyDN*(1 + c2))
		    A(169) = δ*(χsyDN*s1p2)
		    A(170) = 0.5*δ*(χsxDN*(cβ2 - cβ2*c2) + χszDN*(-cβ*sβ + cβ*c2*sβ))
		    A(171) = δ*(χsxDN*cβ*sβ*s2 - χszDN*sβ2*s2)
		    A(172) = 0.5*δ*(χsxDN*(cβ2 + cβ2*c2) + χszDN*(-cβ*sβ - cβ*c2*sβ))
		  ...
		  If Parameters.PNOrder > 2 then
		    ...
		    - 825/8192 *s3β + c2*(27/1024 *sβ - 151/2048 *s3β + 3/2048 *s5β) + c4/8192*(14*sβ + 13*s3β + 15*s5β) + 13/8192*s5β)*s1p4)
		    ...
		    A(78) = δ*(η*((10197/2048 *cβ - 3969/2048 *cβ*c2β)*sβ2 - (1701/2048 *cβ + 5103/2048 *cβ*c2β)*c4*sβ2)*s2p3 _
		    ...
		    + (567/65536 *cβ + 243/131072 *c3β + 1215/131072*c5β)*s10 + η*((5643/4096 *cβ*c2β + cβ*(3195/16384 - 567/16384 *c4β))*s2 _
		    +(-243/1024*cβ*c2β + cβ*(-2259/4096-81/4096*c4β))*s4 _
		    +(-5319/8192*cβ + cβ*(13833/32768-1053/32768*c4β))*s6 _
		    ...
		    A(86) = χszDN*(η*c1p4*(-5/2 - 7/2 *c2β + (1/2 + 1/6 *c2β)*c2) + c1p4*(-3 - c2β + (5 + 5/3 *c2β)*c2)) _
		    ...
		    A(196) = δ*(η*c1p4*(29/512*s2β + c4*(-1/512*s2β+9/1024*s4β) + c2*(13/128*s2β + 11/256*s4β) - 5/1024*s4β)*s1p2 _
		    +c1p4*(355/1024*s2β - c2*(13/256*s2β+11/512*s4β)+c4*(s2β/1024 - 9/2048*s4β) + 5/2048*s4β)*s1p2)
		    A(197) = δ*(c1p5*((-7/96-c2β/48)*sβ2-(1/96+c2β/48)*c2*sβ2)*s1p3 + η*c1p5*((7/48 + c2β/24)*sβ2 + (1/48+c2β/24)*c2*sβ2)*s1p2)
		    A(198) = δ/96*cβ*c1p6*sβ3*s1p4*(2*η-1)
		    A(199) = δ*(η*c1p4*(4923/512 *s2β - c4*(567/1024 *s2β + 1701/2048 *s4β) + 945/2048 *s4β + c2*(459/256 *s2β - 2079/512 *s4β))*s1p2 _
		    +c1p4*(-22203/1024*s2β - 945/2048*s4β + c4*(567/1024*s2β+1701/2048*s4β) + c2*(-459/256*s2β + 2079/512*s4β))*s1p2)
		    A(200) = δ*(η*c1p5*(27/16 + 1233/128 *c2β + 27/128 *c4β - (27/8 + 27/16 *c2β + 27/16 *c4β)*c2 - (81/128 *c2β + 243/128 *c4β)*c4)*s1 _
		    + c1p5*(-27/32 - 4689/256 *c2β - 27/256 *c4β + (27/16 + 27/32 *c2β + 27/32 *c4β)*c2 + (81/256 *c2β + 243/256 *c4β)*c4)*s1)
		    A(201) = δ*(c1p6*(11673/2048 *s2β + c4*(243/2048 *s2β - 2187/4096 *s4β) + c2*(-837/512 *s2β + 621/1024 *s4β) - 1377/4096 *s4β) _
		    + η*c1p6*(-4761/1024 *s2β + c2*(837/256 *s2β - 621/512 *s4β) + 1377/2048 *s4β + c4*(-243/1024 *s2β + 2187/2048*s4β)))
		    A(202) = δ*(c1p7*((-81/64 + 27/32 *c2β)*sβ2 - (81/64 + 81/32 *c2β)*c2*sβ2)*s1  _
		    + η*c1p7*((81/32 - 27/16 *c2β)*sβ2 + (81/32 + 81/16 *c2β)*c2*sβ2)*s1)
		    A(203) = δ*(81/32 *η - 81/64)*cβ*c1p8*sβ3*s1p2
		    A(204) = δ*(4375/384 *s2β + 4375/256 *s4β - η*(4375/192 *s2β + 4375/128 *s4β))*c1p6*s1p4
		    A(205) = δ*(625/96 *c2β + 625/32 *c4β - η*(625/48 *c2β + 625/16 *c4β))*c1p7*s1p3
		    A(206) = δ*(625/256 *s2β - 5625/512 *s4β + η*(-625/128 *s2β + 5625/256 *s4β))*c1p8*s1p2
		    A(207) = δ*(625/96 + 625/48 *c2β - η*(625/48 + 625/24 *c2β))*c1p9*sβ2*s1
		    A(208) = δ*(625/96 *η - 625/192)*cβ*c1p10*sβ3
		    
		    //HX 3/2, SO
		    
		    A(209) = χsyDN*(2*c2p3*sβ - η*c2p3*sβ)
		    A(210) = -2/3*χsyDN*η*c1p4*sβ*(7+2.5*η-c2*(10+η))
		    A(211) = -2/3*χsyDN*cβ*c1p5*s1*(10+η)
		    A(212) = 1/3*χsyDN*c1p3*s1*cβ*(-2+7*η+c2*(10+η))
		    A(213) = -1/3*χsyDN*c1*cβ*s1p3*(2-7*η+c2*(10+η))
		    A(214) = 2/3*χsyDN*s1p4*sβ*(7+2.5*η + c2*(10 + η))
		    A(215) = -2/3*χsyDN*cβ*c1*s1p5*(10+η)
		    A(216) = χsyDN*c2*sβ*s2p2*(2-η)
		    A(217) = 1/3*χsyDN*c2*sβ*s2p2*(10+η)
		    A(218) = -0.5*χsyDN*cβ*s2p3*(2-η)
		    A(219) = -1/8*χsyDN*cβ*(5*s2+s6)*(2-η)
		    A(220) = -s2*(2-η)*(χsxDN*cβ*(0.75+0.25*c4) + χszDN*c4*sβ)
		    A(221) = c2*s2p2*(2-η)*(χszDN*cβ - χsxDN*sβ)
		    A(222) = 0.5*χsxDN*cβ*s2p3*(2-η)
		    A(223) = (χsxDN*(c1*(-2/3 *cβ - 10/3 *cβ*c2)*s1p3 + η*c1*(-5/3 *cβ + 4*c3β - 1/3 *cβ*c2)*s1p3) _
		    + χszDN*(c1*(-4*sβ - 40/3 *c2*sβ)*s1p3 + η*c1*(-2*sβ - 4/3 *c2*sβ - 4*s3β)*s1p3))
		    A(224) = χszDN*(η*(5*cβ + c3β + 2/3 *cβ*c2)*s1p4 + (4*cβ + 20/3 *cβ*c2)*s1p4) _
		    + χsxDN*((-14/3 *sβ - 20/3 *c2*sβ)*s1p4 + η*(10/3 *sβ - 2/3 *c2*sβ + s3β)*s1p4)
		    A(225) = 2/3*χsxDN*cβ*c1*s1p5*(10+η)
		    A(226) = -6*η*χszDN*cβ*sβ2*s2p2 + χsxDN*sβ*s2p2*(1/3  + η*(-7/6 + 3*c2β))
		    A(227) = χsxDN*(η*c1p3*(-5/3 *cβ + 4*c3β + 1/3 *cβ*c2)*s1 + c1p3*(-2/3 *cβ + 10/3 *cβ*c2)*s1) _
		    + χszDN*(c1p3*(-4*sβ + 40/3 *c2*sβ)*s1 + η*c1p3*(-2*sβ + 4/3 *c2*sβ - 4*s3β)*s1)
		    A(228) = χszDN*(η*c1p4*(-5*cβ - c3β + 2/3 *cβ*c2) + c1p4*(-4*cβ + 20/3 *cβ*c2)) _
		    + χsxDN*(c1p4*(14/3 *sβ - 20/3 *c2*sβ) + η*c1p4*(-10/3 *sβ - 2/3 *c2*sβ - s3β))
		    A(229) = 2/3*χsxDN*cβ*c1p5*s1 *(10+η)
		    A(230) = δ*2*χayDN*c2p3*sβ
		    A(231) = δ*χayDN*c1p4*(-14/3 *sβ + 20/3 *c2*sβ)
		    A(232) = δ*χayDN*c1p3*(-2/3 *cβ + 10/3 *cβ*c2)*s1
		    A(233) = -20/3*δ*χayDN*cβ*c1p5*s1
		    A(234) = δ*χayDN*c1*(-2/3 *cβ - 10/3 *cβ*c2)*s1p3
		    A(235) = δ*χayDN*(14/3 *sβ + 20/3 *c2*sβ)*s1p4
		    A(236) = -20/3*δ*χayDN*cβ*c1*s1p5
		    A(237) = 2*δ*χayDN*c2*sβ*s2p2
		    A(238) = 10/3*δ*χayDN*c2*sβ*s2p2
		    A(239) = -δ*χayDN*cβ*s1p3
		    A(240) = -0.25*δ*χayDN*cβ*(5*s2+s6)
		    A(241) = δ*(χaxDN*(-1.5*cβ - 0.5*cβ*c4)*s2 - 2*χazDN*c4*sβ*s2)
		    A(242) = δ*(2*χazDN*cβ*c2*s2p2 - 2*χaxDN*c2*sβ*s2p2)
		    A(243) = δ*χaxDN*cβ*s2p3
		    A(244) = δ*(χaxDN*c1*(-2/3*cβ - 10/3*cβ*c2)*s1p3 + χazDN*c1*(-4*sβ - 40/3*c2*sβ)*s1p3)
		    A(245) = δ*(χazDN*(4*cβ + 20/3 *cβ*c2)*s1p4 + χaxDN*(-14/3*sβ - 20/3*c2*sβ)*s1p4)
		    A(246) = 20/3*δ*χaxDN*cβ*c1*s1p5
		    A(247) = 1/3*δ*χaxDN*sβ*s2p2
		    A(248) = δ*(χaxDN*c1p3*(-2/3*cβ + 10/3*cβ*c2)*s1 + χazDN*c1p3*(-4*sβ + 40/3*c2*sβ)*s1)
		    A(249) = δ*(χazDN*c1p4*(-4*cβ + 20/3*cβ*c2) + χaxDN*c1p4*(14/3 *sβ - 20/3*c2*sβ))
		    A(250) = 20/3*δ*χaxDN*cβ*c1p5*s1
		...
		Sub CalculateWaveFactors()
		  ...
		  For k As Integer = 1 to 5
		    for j as Integer = 1 to 5
		      ...
		      SinAmΨ(j,k)  = (SinApΨ(j,0)*CosApΨ(0,k) - CosApΨ(j,0)*SinApΨ(0,k))
		    Next
